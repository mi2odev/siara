...
export default function PoliceMyIncidentsPage() {
  ...
  return (
    <PoliceShell activeKey="my-incidents" rightPanel={rightPanel} notificationCount={counters.under_review} verificationPendingCount={counters.under_review}>
      <section className="police-section police-my-incidents-page">
        ...
        <div className="police-my-incidents-filters">
          {FILTER_OPTIONS.map((item) => (
            <button key={item.key} className={`police-status-chip ${statusFilter === item.key ? 'active' : ''}`} onClick={() => setStatusFilter(item.key)}>
              {item.label}
            </button>
          ))}
        </div>
          ...
          {filteredIncidents.map((incident) => (
            <article key={incident.id} className="police-my-incident-card">
              ...
              <div className="police-my-incident-actions">
                ...
                <button className="police-action police-action-review" onClick={() => navigate('/police/verification', { state: { incidentId: incident.id } })}>
                  Continue handling
                </button>
              </div>
            </article>
