...
export default function CTA({ title, subtitle, primaryLabel, onPrimary, onSecondary }){
  const { t } = useTranslation(['pages', 'common'])
  const resolvedPrimaryLabel = primaryLabel !== undefined ? primaryLabel : t('common:nav.login')
  return (
    <section style={{padding:'48px 0',display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(90deg,var(--siara-secondary),var(--siara-deep))'}}>
      <div style={{maxWidth:980,display:'flex',flexDirection:'column',alignItems:'center',gap:12,textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:12,marginTop:8}}>
          <Button onClick={onPrimary}>{resolvedPrimaryLabel}</Button>
          <Button variant="ghost" onClick={onSecondary}>{t('cTA.contactUs')}</Button>
        </div>
      </div>
    </section>
  )
