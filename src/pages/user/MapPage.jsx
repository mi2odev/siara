...
export default function MapPage() {
  ...
  const wilayas = ['Alger', 'Oran', 'Constantine', 'Annaba', 'Blida', 'Boumerdès']
  ...
  const hasActiveFilters = severityFilter.length > 0 || typeFilter.length > 0 || selectedWilaya !== 'all' || timeFilter !== '7d'
              ...
              <div className="filter-chips">
                {[
                  ...
                  <button
                    ...
                    style={severityFilter.includes(s.id) ? { background: s.color, borderColor: s.color } : {}}
        ...
        <main className="map-main">
          ...
          <div className="map-container">
            ...
            <div className="map-controls-right">
              <button className="map-ctrl-btn" title="Ma position">
            ...
            <div className="map-canvas">
              <div className="map-bg">
                ...
                {mockMarkers.map(marker => (
                  <div
                    key={marker.id}
                    className={`map-marker severity-${marker.severity}`}
                    style={{
                      left: `${((marker.lng - 3.0) * 500) + 30}%`,
                      top: `${((36.8 - marker.lat) * 500) + 20}%`
                    }}
              ...
              {/* Incident Preview (when selected) */}
              {selectedIncident && (
                <div className="incident-preview">
                  <button className="preview-close" onClick={() => setSelectedIncident(null)}>×</button>
                  <div className={`preview-severity severity-${selectedIncident.severity}`}>
                    {selectedIncident.severity === 'high' ? 'Élevée' : selectedIncident.severity === 'medium' ? 'Moyenne' : 'Faible'}
                  </div>
                  <h4>{selectedIncident.title}</h4>
                  <p className="preview-type">{incidentTypes.find(t => t.id === selectedIncident.type)?.icon} {incidentTypes.find(t => t.id === selectedIncident.type)?.label}</p>
                  <div className="preview-actions">
                    <button className="btn-sm">Voir détails</button>
                    <button className="btn-sm outline">Itinéraire</button>
                  </div>
                </div>
              )}
            </div>
              ...
              {trendingZones.map((zone, i) => (
                <div key={i} className="zone-item">
                  <div className="zone-info">
                    ...
                    <span className="zone-meta">{zone.incidents} incidents • {zone.updated}</span>
                  </div>
              ...
              {activeAlerts.map(alert => (
                <div key={alert.id} className="alert-item">
                  ...
                  <div className="alert-info">
                    ...
                    <span className="alert-time">Il y a {alert.time}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="context-section">
            ...
            <div className="quick-stats">
              ...
              <div className="stat-item">
                ...
                <span className="stat-label">Gravité moy.</span>
              </div>
              <div className="stat-item">
